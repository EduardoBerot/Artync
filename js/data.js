// Cities config — mirrors lib/cities.ts in the Next.js spec
window.ARTYNC_CITIES = {
  "lajeado": {
    slug: "lajeado",
    name: "Lajeado",
    region: "Vale do Taquari",
    state: "RS",
    headlinePrefix: "Para empresas de",
    sub: "Sites, landing pages e sistemas web sob medida para negócios de Lajeado que querem crescer no digital.",
    population: "84 mil",
    nearby: ["Estrela", "Arroio do Meio", "Cruzeiro do Sul"],
  },
  "santa-cruz-do-sul": {
    slug: "santa-cruz-do-sul",
    name: "Santa Cruz do Sul",
    region: "Vale do Rio Pardo",
    state: "RS",
    headlinePrefix: "Para negócios locais de",
    sub: "Desenvolvimento web profissional para empresas de Santa Cruz do Sul que querem ser encontradas no Google.",
    population: "131 mil",
    nearby: ["Vera Cruz", "Venâncio Aires", "Rio Pardo"],
  },
  "arroio-do-meio": {
    slug: "arroio-do-meio",
    name: "Arroio do Meio",
    region: "Vale do Taquari",
    state: "RS",
    headlinePrefix: "Para empresas de",
    sub: "Criamos sites e sistemas que vendem para o comércio e indústria de Arroio do Meio.",
    population: "21 mil",
    nearby: ["Lajeado", "Encantado", "Estrela"],
  },
  "estrela": {
    slug: "estrela",
    name: "Estrela",
    region: "Vale do Taquari",
    state: "RS",
    headlinePrefix: "Para empresas de",
    sub: "Sites e landing pages para empresas de Estrela que querem mais clientes vindos da internet.",
    population: "33 mil",
    nearby: ["Lajeado", "Teutônia", "Cruzeiro do Sul"],
  },
  "venancio-aires": {
    slug: "venancio-aires",
    name: "Venâncio Aires",
    region: "Vale do Rio Pardo",
    state: "RS",
    headlinePrefix: "Para negócios locais de",
    sub: "Tecnologia web aplicada à realidade das empresas de Venâncio Aires.",
    population: "75 mil",
    nearby: ["Santa Cruz do Sul", "Vera Cruz", "Mato Leitão"],
  },
  "teutonia": {
    slug: "teutonia",
    name: "Teutônia",
    region: "Vale do Taquari",
    state: "RS",
    headlinePrefix: "Para empresas de",
    sub: "Sites institucionais, lojas e sistemas web para empresas de Teutônia.",
    population: "32 mil",
    nearby: ["Estrela", "Westfália", "Imigrante"],
  },
  "encantado": {
    slug: "encantado",
    name: "Encantado",
    region: "Vale do Taquari",
    state: "RS",
    headlinePrefix: "Para empresas de",
    sub: "Presença digital sólida para os negócios de Encantado e região.",
    population: "23 mil",
    nearby: ["Roca Sales", "Muçum", "Arroio do Meio"],
  },
  "vera-cruz": {
    slug: "vera-cruz",
    name: "Vera Cruz",
    region: "Vale do Rio Pardo",
    state: "RS",
    headlinePrefix: "Para empresas de",
    sub: "Sites profissionais para empresas de Vera Cruz que querem vender mais no Vale do Rio Pardo.",
    population: "26 mil",
    nearby: ["Santa Cruz do Sul", "Vale Verde", "Passo do Sobrado"],
  },
  "rio-pardo": {
    slug: "rio-pardo",
    name: "Rio Pardo",
    region: "Vale do Rio Pardo",
    state: "RS",
    headlinePrefix: "Para quem quer vender mais em",
    sub: "Desenvolvimento web orientado a resultado para empresas de Rio Pardo.",
    population: "37 mil",
    nearby: ["Santa Cruz do Sul", "Pantano Grande", "Encruzilhada do Sul"],
  },
};

window.ARTYNC_BENEFITS = [
  {
    title: "Sites que aparecem no Google",
    body: "Otimização técnica de SEO incluída em todo projeto. Schema, performance, semântica e conteúdo local pensados para ranquear desde o primeiro mês.",
    visual: "flow",
  },
  {
    title: "Performance de verdade",
    body: "Carregamento sub-segundo, Core Web Vitals no verde e imagens otimizadas com next/image. Velocidade que converte visita em cliente.",
    visual: "slider",
  },
  {
    title: "Conversão acima da média",
    body: "Layouts validados em A/B test, CTAs com gatilhos psicológicos e copy focada em dor. Páginas convertem 3x a média do mercado.",
    visual: "bars",
  },
  {
    title: "Integração com seu fluxo",
    body: "WhatsApp, CRM, Google Ads, Meta, RD Station, e-mail marketing. Tudo conversa entre si. Lead chega já etiquetado.",
    visual: "api",
  },
  {
    title: "Suporte que responde",
    body: "Atendimento direto comigo no WhatsApp. Sem ticket, sem fila, sem chatbot. Mexeu, quebrou, ajustou — em horas, não dias.",
    visual: "team",
  },
  {
    title: "Código que cresce com você",
    body: "Next.js 14, TypeScript e arquitetura modular. Quando seu negócio crescer, o site escala junto — sem precisar refazer do zero.",
    visual: "scale",
  },
];

window.ARTYNC_STEPS = [
  {
    n: "01",
    title: "Conversa de descoberta",
    desc: "Entendo seu negócio, público, concorrentes e metas. 30 min de bate-papo no WhatsApp ou Meet — gratuito e sem compromisso.",
    chips: ["Briefing", "Análise de concorrentes", "Definição de metas"],
  },
  {
    n: "02",
    title: "Estratégia e protótipo",
    desc: "Recebo o conteúdo, defino a arquitetura e monto um protótipo navegável. Você aprova antes de qualquer linha de código.",
    chips: ["Wireframe", "Protótipo Figma", "Copy estratégica"],
  },
  {
    n: "03",
    title: "Desenvolvimento e testes",
    desc: "Codifico em Next.js com SEO técnico, performance e responsividade. Acompanha tudo num link de pré-visualização ao vivo.",
    chips: ["Next.js 14", "SEO técnico", "Preview ao vivo"],
  },
  {
    n: "04",
    title: "Lançamento e crescimento",
    desc: "Publico no ar, configuro Google Analytics, Search Console e Tag Manager. 30 dias de ajustes inclusos para afinar conversão.",
    chips: ["Deploy", "Analytics", "Otimização contínua"],
  },
];

window.ARTYNC_TESTIMONIALS = [
  { name: "Carla Bender", role: "Imobiliária Bender — Lajeado", quote: "Em 3 meses o site já estava na primeira página do Google para 'imobiliária Lajeado'. As ligações de cliente não pararam mais.", stars: 5 },
  { name: "Roberto Klein", role: "Klein Materiais — Estrela", quote: "Saímos de zero pedidos online para 40 por semana. O sistema de orçamento integrado ao WhatsApp foi um divisor de águas.", stars: 5 },
  { name: "Patrícia Mertens", role: "Clínica Mertens — Santa Cruz do Sul", quote: "Profissionalismo, prazo cumprido e o site lindo. Mas o melhor é que o suporte responde rápido quando preciso ajustar algo.", stars: 5 },
  { name: "Eduardo Schmitt", role: "Schmitt Advocacia — Venâncio Aires", quote: "A landing page de captação dobrou os agendamentos do escritório no primeiro mês. Investimento que pagou sozinho.", stars: 5 },
  { name: "Juliana Müller", role: "Estética Müller — Teutônia", quote: "O agendamento online direto pelo site cortou 80% do tempo gasto no telefone. A equipe agradece todo dia.", stars: 5 },
  { name: "Marcos Hentges", role: "Hentges Pneus — Arroio do Meio", quote: "Nunca tive site antes. Hoje 60% dos clientes novos chegam pelo Google. Vale cada centavo.", stars: 5 },
  { name: "Larissa Oliveira", role: "Belle Boutique — Encantado", quote: "Loja virtual integrada ao Instagram e WhatsApp. Triplicou as vendas online em 4 meses.", stars: 5 },
  { name: "Fernando Reck", role: "Reck Construções — Rio Pardo", quote: "Sistema interno de obras feito sob medida. Economiza 10 horas da minha semana. Não volto mais para planilha.", stars: 5 },
];

window.ARTYNC_FAQ = [
  {
    q: "Quanto custa um site?",
    a: "Depende do escopo, mas eu trabalho com 3 faixas: site institucional a partir de R$ 2.800, landing page de alta conversão a partir de R$ 1.900 e sistemas web sob medida com orçamento personalizado. Toda proposta é fechada após o briefing — sem letras miúdas, sem custo escondido.",
  },
  {
    q: "Vocês atendem minha cidade?",
    a: "Atendo presencialmente toda a região do Vale do Taquari e Vale do Rio Pardo (Lajeado, Estrela, Arroio do Meio, Encantado, Teutônia, Santa Cruz do Sul, Vera Cruz, Venâncio Aires, Rio Pardo e cidades vizinhas). Para outras regiões, atendo 100% online — o resultado é o mesmo.",
  },
  {
    q: "O site aparece no Google?",
    a: "Sim. Todo projeto sai com SEO técnico configurado: meta tags, sitemap, robots, dados estruturados (Schema.org), Core Web Vitals otimizado e conteúdo pensado para palavras-chave locais. Ranqueamento começa em poucas semanas e só cresce com o tempo.",
  },
  {
    q: "Em quanto tempo o site fica pronto?",
    a: "Landing page: 7 a 10 dias úteis. Site institucional: 3 a 4 semanas. Sistema web sob medida: a partir de 6 semanas. Tudo isso depois do briefing aprovado e do conteúdo recebido — quanto mais ágil você for, mais rápido entrega no ar.",
  },
  {
    q: "Eu consigo editar o site depois?",
    a: "Consegue. Entrego com painel administrativo simples para você editar textos, fotos e novidades. Quando precisar de algo mais técnico, me chama no WhatsApp — atendo direto, sem fila de suporte.",
  },
  {
    q: "Por que não usar Wix ou WordPress?",
    a: "Construtores de site são ótimos para quem está começando. Mas conforme você cresce, eles travam: ficam lentos, ranqueiam mal, são difíceis de personalizar e cobram caro por extensões. Site profissional em Next.js é mais rápido, mais seguro e ranqueia melhor.",
  },
];
