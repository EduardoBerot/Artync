...
│   └── CLAUDE.md           # Contexto técnico para sessões com Claude
├── docs/
...
`js/app.jsx` define `TWEAK_DEFAULTS` dentro de marcadores `/*EDITMODE-BEGIN*/...{ }.../*EDITMODE-END*/`. O host (Codex artifacts/preview) reescreve esse bloco no **source `.jsx`** quando o usuário ajusta o painel — não remover os marcadores. Após edit, é preciso rodar `npm run build` para refletir no `dist/`.
